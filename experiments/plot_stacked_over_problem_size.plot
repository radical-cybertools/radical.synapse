...
do for [term_i=1:words(terms)] {
  ...
  # set title     'synapse profiling vs. emulation [boskop]'
    set logscale xy
  ...
  # set size 1.0,1.5
    ...
    set xtics  in
    set ytics  in 1,10,1000
    set xlabel ''
    set ylabel "TTC\n[sec]" offset second -0.06,0
    set format x ""

    plot [5:10000][0.5:5000] \
        '<(grep -e "STAT_RUN" '.dat.')' using 3:7       title ''                      with lines       ls 6, \
        '<(grep -e "STAT_RUN" '.dat.')' using 3:7:8     title 'Execution'             with yerrorbars  ls 6, \
